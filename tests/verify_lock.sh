...
run_it() { ( cd "$W" && RTC_AWAKE=1 RTC_RETRY_WAITS="1 1" bash scripts/run_collect.sh >/dev/null 2>&1 ); echo $?; }
...
echo
echo "--- 6b. ジャンル名を指定したとき・省略したとき ---"
setup
echo 0 > "$W/FAIL_TIMES"
# macOS 標準の bash 3.2 は、空の配列を set -u 下で "${arr[@]}" と書くと落ちる。
# launchd は必ずジャンル名を渡すので表面化していなかった（実際に踏んだ）
STATUS="$( ( cd "$W" && RTC_AWAKE=1 RTC_RETRY_WAITS="1 1" bash scripts/run_collect.sh >/dev/null 2>&1 ); echo $? )"
check "ジャンル名を省いても動く" "$STATUS" "0"
check "省いたときは全ジャンルと記録される" "$(contains "$W/logs/collect.log" "開始 全ジャンル")" "yes"
teardown

setup
echo 0 > "$W/FAIL_TIMES"
STATUS="$( ( cd "$W" && RTC_AWAKE=1 RTC_RETRY_WAITS="1 1" bash scripts/run_collect.sh 育毛 >/dev/null 2>&1 ); echo $? )"
check "ジャンル名を渡しても動く" "$STATUS" "0"
check "渡したジャンル名が記録される" "$(contains "$W/logs/collect.log" "開始 育毛")" "yes"
teardown
